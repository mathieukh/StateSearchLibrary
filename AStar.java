...
import Model.OrientedSearchModel.Heuristic;
import Model.OrientedSearchModel.OrientedSearchAlgorithm;
...
/**
 * Created by sylom on 14/01/2016.
 */
public class AStar extends OrientedSearchAlgorithm {
    ...
    public Solution findSolution(State initial) throws NoSolutionException {
        ...
        while (!openList.isEmpty())
        {
            ...
            if (u.state.isTerminal()) return makeSolution(u);
            for(State v : u.state.successors())
            {
                Node vn = new Node(v,u);
                if(existsWithLowestCost(vn,closedList) || existsWithLowestCost(vn,openList)) continue;
                else
                {
    ...
    private boolean existsWithLowestCost(Node n, Collection<Node> list)
    {
        if(!list.contains(n)) return false;
            ...
            for(Node c : list)
                if(c.equals(n)) return c.cost < n.cost;
        return false;
    }
